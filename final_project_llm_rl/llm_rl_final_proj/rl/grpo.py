    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                log_ratio = torch.clamp(new_logp - mb.old_logprobs, min=-20, max=20) # [B, L - 1]
                ratio = torch.exp(log_ratio) # [B, L - 1]

                # [B, 1]
                adv = adv.unsqueeze(1)
                clipped_ratio = torch.clamp(ratio, min=1.0 - cfg.clip_eps, max=1.0 + cfg.clip_eps) # [B, L - 1]
                per_token_obj = torch.min(ratio * adv, clipped_ratio * adv) * mask # [B, L - 1]
                seq_obj = masked_mean_per_row(per_token_obj, mask) # [B]
                pg_loss = - seq_obj.mean() # scalar
                ...
                clipfrac = masked_mean(is_clipped.to(new_logp.dtype), mask)
