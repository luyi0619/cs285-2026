    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                seq_log_ratio = masked_mean_per_row(new_logp - mb.old_logprobs, mask) # [B]
                seq_ratio = torch.exp(seq_log_ratio) # [B]
                clipped_seq_ratio = torch.clamp(seq_ratio, min=1.0 - cfg.clip_eps, max=1.0 + cfg.clip_eps) # [B]
                seq_obj = torch.min(seq_ratio * adv, clipped_seq_ratio * adv) # [B]
                pg_loss = - seq_obj.mean() # scalar
                kl = approx_kl_from_logprobs(new_logp, mb.ref_logprobs, mask)
                entropy = -masked_mean(new_logp, mask)

                is_clipped = (seq_ratio < 1 - cfg.clip_eps) | (seq_ratio > 1 + cfg.clip_eps)
                clipfrac = is_clipped.to(new_logp.dtype).mean()
