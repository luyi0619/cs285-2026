...
#### Local training
```bash
uv run python scripts/train.py reward_model \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --dataset_name ./dataset/wildchat_min4_judged_5k_v1 \
...
For local IPO training, use the same command with `--algo ipo`, `--output_dir ./runs/wildchat_min4_judged_5k_ipo_v1`, and `--wandb_name wildchat_min4_judged_5k_ipo_v1`.

For local AOT training, use the same command with `--algo aot`, `--output_dir ./runs/wildchat_min4_judged_5k_aot_beta02_v1`, `--wandb_name wildchat_min4_judged_5k_aot_beta02_v1`, `--beta 0.2`, `--generation_eval_every 50`, `--eval_interval 50`, and `--save_interval 50`.

For Modal training, replace dataset and output paths with their `/vol/` equivalents (e.g., `--dataset_name /vol/synthetic_datasets/wildchat_min4_judged_5k_v1`, `--output_dir /vol/runs/...`).

### Online RLHF training
#### Local training
```bash
uv run python scripts/train.py rm_grpo \
  --algo grpo \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --dataset_name ./dataset/wildchat_min4_judged_5k_v1 \
  ...
  --reward_adapter_path ./runs/wildchat_min4_judged_5k_reward_model_v1/checkpoints/step_000445/adapter \
...
Use the same command for `dr_grpo` and `gspo`, changing `--algo`, `--output_dir`, and `--wandb_name`. For Part 1 online policy submissions, use checkpoint `step_000025`.
...
| File | Local Adapter path | Modal Adapter path |
| --- | --- | --- |
| `policy_generations/dpo.jsonl` | `./runs/wildchat_min4_judged_5k_dpo_beta01_v1/checkpoints/step_000100/adapter` | `/vol/runs/wildchat_min4_judged_5k_dpo_beta01_v1/checkpoints/step_000100/adapter` |
| `policy_generations/ipo.jsonl` | `./runs/wildchat_min4_judged_5k_ipo_v1/checkpoints/step_000300/adapter` | `/vol/runs/wildchat_min4_judged_5k_ipo_v1/checkpoints/step_000300/adapter` |
| `policy_generations/aot.jsonl` | `./runs/wildchat_min4_judged_5k_aot_beta02_v1/checkpoints/step_000550/adapter` | `/vol/runs/wildchat_min4_judged_5k_aot_beta02_v1/checkpoints/step_000550/adapter` |
| `policy_generations/grpo.jsonl` | `./runs/wildchat_min4_judged_5k_grpo_rm445_v1/checkpoints/step_000025/adapter` | `/vol/runs/wildchat_min4_judged_5k_grpo_rm445_v1/checkpoints/step_000025/adapter` |
| `policy_generations/drgrpo.jsonl` | `./runs/wildchat_min4_judged_5k_drgrpo_rm445_v1/checkpoints/step_000025/adapter` | `/vol/runs/wildchat_min4_judged_5k_drgrpo_rm445_v1/checkpoints/step_000025/adapter` |
| `policy_generations/gspo.jsonl` | `./runs/wildchat_min4_judged_5k_gspo_rm445_v1/checkpoints/step_000025/adapter` | `/vol/runs/wildchat_min4_judged_5k_gspo_rm445_v1/checkpoints/step_000025/adapter` |
| `reward_model/public_test_pref_scores.jsonl` | `./runs/wildchat_min4_judged_5k_reward_model_v1/checkpoints/step_000445/adapter` | `/vol/runs/wildchat_min4_judged_5k_reward_model_v1/checkpoints/step_000445/adapter` |
...
- `./public_eval/public_test_gen_prompts_128.jsonl` (local)
- `/root/project/public_eval/public_test_gen_prompts_128.jsonl` (Modal)

#### Local submission
```bash
uv run python llm_rl_final_proj/build_policy_submission.py \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path ./runs/<run>/checkpoints/<step>/adapter \
  --prompts_jsonl ./public_eval/public_test_gen_prompts_128.jsonl \
  --output_jsonl ./submissions/dpo.jsonl \
  --max_prompt_tokens 700 \
  --max_new_tokens 256 \
  --temperature 0.0 \
  --top_p 1.0
...
- `./public_eval/public_test_prefs_256.jsonl` (local)
- `/root/project/public_eval/public_test_prefs_256.jsonl` (Modal)

The autograder computes pair accuracy from the uploaded `chosen_score` and `rejected_score` values.

#### Local submission
```bash
uv run python llm_rl_final_proj/build_reward_model_submission.py \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path ./runs/<reward_model_run>/checkpoints/<step>/adapter \
  --prefs_jsonl ./public_eval/public_test_prefs_256.jsonl \
  --output_jsonl ./submissions/public_test_pref_scores.jsonl \
  --max_prompt_tokens 700 \
  --max_response_tokens 512
```
